...
void GcodeSuite::get_destination_from_command() {
  ...
  #if ENABLED(POWER_LOSS_RECOVERY) /*&& !PIN_EXISTS(POWER_LOSS)*/  //ZB  cause  powerloss_failure
    // Only update power loss recovery on moves with E
    if (recovery.enabled && IS_SD_PRINTING() && seen.e && (seen.x || seen.y))
      recovery.save();
  #endif

  if (parser.linearval('F') > 0)//modify of david ,for changes default freedrate_mm_s in fast_print_mode
  {
  	if(!fast_print_enable)   feedrate_mm_s = parser.value_feedrate();
	 // else					 feedrate_mm_s = 50;
  }
  ...
  //do{ Serial.print("feedrate_mm_s.x="); Serial.println(feedrate_mm_s); }while(0);
  #if ENABLED(PRINTCOUNTER)
