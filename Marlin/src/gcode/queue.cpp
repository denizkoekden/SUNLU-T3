...
#if 1
          // if (strstr(command.buffer, "WALL-OUTER") || strstr(command.buffer, "outer perimeter")) {
          if (strstr(command.buffer, "WALL-OUTER")) {
            W_Flag = true;
            Q_Flag = false;
          }
          else if (strstr(command.buffer, ";TYPE:")) {

            Q_Flag = true;
            W_Flag = false;
          }

          if (W_Flag) { strcat(command.buffer, "W"); }
          if (Q_Flag) { strcat(command.buffer, "Q"); }
#endif
