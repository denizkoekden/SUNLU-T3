...
extern bool G29_Complete;
extern bool displayNameFlag;
...
void Auto_level_Swtich(bool TmpZ)
{
	uint8_t save_state=0;
	do{ Serial.println("Auto_level_Swtich111:"); }while(0);

	planner.synchronize();
	save_state = IS_SD_PRINTING();

	do{ Serial.print("IS_SD_PRINTING="); Serial.println(save_state); }while(0);
  do{ Serial.print("auto_level_enable1111="); Serial.println(auto_level_enable); }while(0);
	if(IS_SD_PRINTING()&&(auto_level_enable)&&TmpZ)
	{
	
	//do{ Serial.print("planner.leveling_active="); Serial.println(planner.leveling_active); }while(0);
	...
	//if(IS_SD_PRINTING()) TERN(G29_RETRY_AND_RECOVER, G29_with_retry, G29)();
...
void GcodeSuite::G28() {
  ...
  Stepper::AdjusHomeSpeedFlag = true;
  ...
  do{ Serial.print("planner.leveling_active="); Serial.println(planner.leveling_active); }while(0);
  do{ Serial.print("doZ="); Serial.println(doZ); }while(0);
  Auto_level_Swtich(doZ);


  report_current_position();//end
...
#if 1
  if(IS_SD_PRINTING() && !auto_level_enable)  displayNameFlag = true;
#endif

  do{ Serial.println("Auto_level_Swtichaaa:"); }while(0);
  Stepper::AdjusHomeSpeedFlag = false;
  max_point[1]=max_point[0]=-999.0;
  min_point[1]=min_point[0]=999.0;
  G29_Complete =false;
  //TERN(G29_RETRY_AND_RECOVER, G29_with_retry, G29)();
  
  //if(IS_SD_PRINTING()) queue.enqueue_one_P(PSTR("G29"));
 // queue.enqueue_one_P(PSTR("G29"));
//if(ExtUI::isPrintingFromMedia()) TERN(G29_RETRY_AND_RECOVER, G29_with_retry, G29)();
}
