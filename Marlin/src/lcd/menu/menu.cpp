...
void scroll_screen(const uint8_t limit, const bool is_menu) {
  ...
  //do{ Serial.print("lcd_babystep_zoffset"); }while(0);    
  if(card.isPrinting()){
  	do_G28 =do_G28;
  	}
  else{
	  if(!do_G28)
	  {
		...
		planner.synchronize();
		queue.enqueue_now_P(PSTR("G0 X115 G0 Y115"));
		planner.synchronize();
		queue.enqueue_now_P(PSTR("G0 Z0"));
		planner.synchronize();
		//ui.manual_move.menu_scale = 10;
  ...
  if (ui.use_click())
  {
      ...
      //settings.eeprom_save_par = true;
	do{ Serial.println("lcd_babystep_zoffset_save_eeprom"); }while(0);	  
  return ui.goto_previous_screen_no_defer();
    ...
    if (ui.encoderPosition) {
		...
		do{ Serial.println("babystep111"); }while(0);	
      ...
      if (WITHIN(new_offs, Z_PROBE_OFFSET_RANGE_MIN, Z_PROBE_OFFSET_RANGE_MAX)) {
		...
		do{ Serial.println("babystep222"); }while(0);	
    ...
    if (ui.should_draw()) {
		...
		do{ Serial.println("babystep333"); }while(0);	
      if (do_probe) {
	  	
		do{ Serial.println("babystep444"); }while(0);	
