...
#endif
extern uint8_t Filament_Runout_AutoRecovery_Cnt ;
 ...
 */
  ...
  void menu_change_filament() {
    ...
    bool tmpFast_print_enable = fast_print_enable; // zcj
    
    START_MENU();
    BACK_ITEM(MSG_MAIN);
    if (fast_print_enable) fast_print_enable = false; // zcj
    // Change filament
    #if E_STEPPERS == 1
      ...
      if (!is_busy) {
        // Load filament
          ...
          if (thermalManager.targetTooColdToExtrude(active_extruder)){
		  	//do{ Serial.println("load_MSG_FILAMENTLOAD111");  }while(0);
            SUBMENU_P(msg_load, []{ _menu_temp_filament_op(PAUSE_MODE_LOAD_FILAMENT, 0); });
          	}
          else{
		  	
		  	//do{ Serial.println("load_MSG_FILAMENTLOAD222");  }while(0);
            GCODES_ITEM_P(msg_load, PSTR("M701"));//M701 L50
          	}
        #else
        ...
        // Unload filament
          ...
          if (thermalManager.targetTooColdToExtrude(active_extruder)){
		  	...
		  	//do{ Serial.println("unload_MSG_FILAMENTLOAD111");  }while(0);
            SUBMENU_P(msg_unload, []{ _menu_temp_filament_op(PAUSE_MODE_UNLOAD_FILAMENT, 0); });
          	}
          else{
		  	
		  	//do{ Serial.println("unload_MSG_FILAMENTLOAD222");  }while(0);
            GCODES_ITEM_P(msg_unload, PSTR("M702"));//M702 U%d
          	}
        #else
    ...
    END_MENU();
    fast_print_enable = tmpFast_print_enable;
  }
#endif
...
void menu_pause_option() {
  ...
  #endif
  //ACTION_ITEM(MSG_FILAMENT_CHANGE_OPTION_PURGE, []{ pause_menu_response = PAUSE_RESPONSE_EXTRUDE_MORE; });//zb delete
  ...
  if (!still_out){
     //do{ Serial.println("menu_pause_option>>>>>");  }while(0);
     runout.clear_LCD_error_out();
  	pause_menu_response = PAUSE_RESPONSE_RESUME_PRINT;
	Filament_Runout_AutoRecovery_Cnt =0;
...
void lcd_pause_heat_message()     {
  if(runout.get_LCD_error_out()) {
    if(runout.get_LCD_error_out() ==1){
       _lcd_pause_message(GET_TEXT(MSG_FILAMENT_CHANGE_FIAM));  
    }
    else if(runout.get_LCD_error_out() ==2){
       _lcd_pause_message(GET_TEXT(MSG_FILAMENT_CHANGE_JAMS));
    }
   // runout.clear_LCD_error_out();
  }
  else{
     _lcd_pause_message(GET_TEXT(MSG_FILAMENT_CHANGE_HEAT)); 
  }
...
void lcd_pause_waiting_message()  {
  //uint8_t  get_filament_state = runout.get_LCD_error_out();
 // do{ Serial.print("runout.get_LCD_error_out="); Serial.println(get_filament_state); }while(0);

  if(runout.get_LCD_error_out()) {
    if(runout.get_LCD_error_out() ==1){
       _lcd_pause_message(GET_TEXT(MSG_ADVANCED_PAUSE_FIAM));  
    }
    else if(runout.get_LCD_error_out() ==2){
       _lcd_pause_message(GET_TEXT(MSG_ADVANCED_PAUSE_JAMS));
    }
   // runout.clear_LCD_error_out();
  }
  else{
     _lcd_pause_message(GET_TEXT(MSG_ADVANCED_PAUSE_WAITING)); 
  }
}
