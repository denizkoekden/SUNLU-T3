  ...
  FORCE_INLINE void _draw_hotend_status(const heater_id_t heater_id, const bool blink) {
    ...
    if (PAGE_CONTAINS(STATUS_HEATERS_Y, STATUS_HEATERS_BOT)) {
      ...
      NOMORE(tall, BAR_TALL);
        ...
        #endif
            //u8g.drawBitmapP(0, 2, 3, 17,T3_LOGO_bmp );//zb  yes
            u8g.drawBitmapP(0, 2, STATUS_LOGO_BYTEWIDTH, STATUS_LOGO_HEIGHT,T3_LOGO_bmp );//zb  yes
            u8g.drawBitmapP(52, STATUS_HEATERS_Y, bw, STATUS_HEATERS_HEIGHT, HOTEND_BITMAP(TERN(HAS_MMU, active_extruder, heater_id), isHeat));//zb nozzle 
      #endif
    ...
    if (PAGE_UNDER(7)) {
      ...
      #endif
      if (dodraw){
      //_draw_centered_temp(target, tx, 7);
         _draw_centered_temp(target, 70-11, 7);//zb temp1
      }
    }

    if (PAGE_CONTAINS(28 - INFO_FONT_ASCENT, 28 - 1)){
      //_draw_centered_temp(temp, tx, 28);
      _draw_centered_temp(temp, 70-11, 28);//zb temp2
    }
...
void MarlinUI::draw_status_screen() {
  ...
  set_font(FONT_STATUSMENU);
    ...
    if (PAGE_CONTAINS(STATUS_FAN_Y, STATUS_FAN_Y + STATUS_FAN_HEIGHT - 1))
      u8g.drawBitmapP(STATUS_FAN_X+2, STATUS_FAN_Y, STATUS_FAN_BYTEWIDTH, STATUS_FAN_HEIGHT,
        #if STATUS_FAN_FRAMES > 2
  ...
  if (PAGE_UNDER(6 + 1 + 12 + 1 + 6 + 1)) {
    ...
    // Fan, if a bitmap was provided
      ...
      if (PAGE_CONTAINS(STATUS_FAN_TEXT_Y - INFO_FONT_ASCENT, STATUS_FAN_TEXT_Y - 1)) {
        ...
        if (spd) {
          ...
          #endif
          lcd_put_u8str(STATUS_FAN_TEXT_X+2, STATUS_FAN_TEXT_Y, i16tostr3rj(thermalManager.pwmToPercent(spd)));
          lcd_put_wchar(c);
        }
      }
    #endif
    ...
    if (card.isFileOpen() && PAGE_CONTAINS(42, 51)) {
      // Upper box
      u8g.drawBox(42, 41, 8, 7);     // 42-48 (or 41-47)
      // Right edge
      u8g.drawBox(50, 43, 2, 5);     // 44-48 (or 43-47)
      // Bottom hollow box
      u8g.drawFrame(42, 48, 10, 4);  // 49-52 (or 48-51)
      // Corner pixel
      u8g.drawPixel(50, 42);         // 43 (or 42)
    }
  #endif // SDSUPPORT
    ...
    if (PAGE_CONTAINS(PROGRESS_BAR_Y, PROGRESS_BAR_Y + 3))
      u8g.drawFrame(PROGRESS_BAR_X, PROGRESS_BAR_Y, PROGRESS_BAR_WIDTH, 3);
    ...
    if (PAGE_CONTAINS(PROGRESS_BAR_Y + 1, PROGRESS_BAR_Y + 2))
      u8g.drawBox(PROGRESS_BAR_X + 1, PROGRESS_BAR_Y + 1, progress_bar_solid_width, 1);
    ...
    if (PAGE_CONTAINS(EXTRAS_BASELINE - INFO_FONT_ASCENT, EXTRAS_BASELINE - 1)) {
        ...
        #endif
            lcd_put_u8str(elapsed_x_pos, EXTRAS_BASELINE, elapsed_string);
            if(fast_print_enable)            lcd_put_u8str(128-5, EXTRAS_BASELINE, "F");
            else                              lcd_put_u8str(128-5, EXTRAS_BASELINE, "N");
  ...
  if (PAGE_CONTAINS(STATUS_BASELINE - INFO_FONT_ASCENT, STATUS_BASELINE + INFO_FONT_DESCENT)) {
  	//if(ui.language == 0)	lcd_moveto(0, STATUS_BASELINE+1);
	//else					
	lcd_moveto(0, STATUS_BASELINE+1);
