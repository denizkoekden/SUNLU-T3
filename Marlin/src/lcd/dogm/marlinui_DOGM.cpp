...
void MarlinUI::clear_lcd() { } // Automatically cleared by Picture Loop
    ...
    if (mark_as_selected(row, style & SS_INVERT)) {
      ...
      if (style & SS_CENTER) {
        ...
        if(ui.language ==0){
           ...
           pad = ((LCD_WIDTH - plen - vlen) / 2)-2;  //zb
        }
        else{
           pad = ((LCD_WIDTH - plen - vlen) / 2);
        }
  ...
  void MenuEditItemBase::draw_edit_screen(PGM_P const pstr, const char * const value/*=nullptr*/) {
	...
	//do{ Serial.println("_lcd_zoffset_overlay_gfxbbb"); }while(0);	
  ...
  inline void draw_boxed_string(const u8g_uint_t x, const u8g_uint_t y, PGM_P const pstr, const bool inv) {
    ...
    if (inv) {
      if(ui.language ==0){
        //u8g.setColorIndex(1);
        u8g.drawBox(bx / prop - 1, by - (MENU_FONT_ASCENT) + 1-1, bw / prop + 2+15, MENU_FONT_HEIGHT - 1);
        u8g.setColorIndex(0);
      }
      else{
        u8g.setColorIndex(1);
  ...
  void MenuItem_confirm::draw_select_screen(PGM_P const yes, PGM_P const no, const bool yesno, PGM_P const pref, const char * const string/*=nullptr*/, PGM_P const suff/*=nullptr*/) {
    ...
    if(ui.language == 0){
      draw_boxed_string(xpos - (utf8_strlen_P(yes) + 1)+1, LCD_HEIGHT - 1, yes, yesno);
    }
    else{
    ...
    void _lcd_zoffset_overlay_gfx(const_float_t zvalue) {
		//do{ Serial.println("_lcd_zoffset_overlay_gfx111"); }while(0);	
