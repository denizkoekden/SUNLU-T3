...
#define NEW_LCD_UPDATE_INTERVAL 300
...
void MarlinUI::update() {
  ...
  if (ELAPSED(ms, next_lcd_update_ms) || TERN0(HAS_MARLINUI_U8GLIB, drawing_screen)) {
	  ...
	  if(IS_SD_PRINTING())
      next_lcd_update_ms = ms + NEW_LCD_UPDATE_INTERVAL;
    else
  ...
  void MarlinUI::update_buttons() {
    ...
    //if (ELAPSED(now, next_button_update_ms)) 
	{
        ...
        #endif // UP || DWN || LFT || RT
        if((newbutton &EN_C)==EN_C)
        {
          if(old_button_state ^(newbutton &EN_C))
          {
            //do{ Serial.print("buttons value="); Serial.println(buttons); }while(0);
            if (ELAPSED(now, next_button_update_ms)) 
            {
  ...
  void MarlinUI::reset_status(const bool no_welcome) {
    ...
    if (printingIsPaused())
      ...
      msg = GET_TEXT(WELCOME_MSG);
			//do{ Serial.print("reset_status_ui_language="); Serial.println(ui.language); }while(0);  
    	}
    else
      return;
  ...
  void MarlinUI::resume_print() {
	...
	Filament_Runout_AutoRecovery_Cnt = 0;
	do{ Serial.println("resume_print222");  }while(0);
    reset_status();
  ...
  void MarlinUI::media_changed(const uint8_t old_status, const uint8_t status) {
    ...
    #endif
	  if(IS_SD_PRINTING())
      TERN_(HAS_WIRED_LCD, next_lcd_update_ms = ms + NEW_LCD_UPDATE_INTERVAL); // Delay LCD update for SD activity
    else
