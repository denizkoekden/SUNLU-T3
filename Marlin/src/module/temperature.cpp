...
static uint16_t  TmpErrorCnt =0;
#define TmpErrorNum 1000
...
// public:
  ...
  void Temperature::set_fan_speed(uint8_t fan, uint16_t speed) {
	...
	//do{ Serial.print("FAN_COUNT:"); Serial.println(FAN_COUNT); }while(0);

    if (fan >= FAN_COUNT) return;
	//do{ Serial.print("set_fan_speed_fan="); Serial.println(fan); }while(0);
          ...
          if (BOTH(WATCH_BED, WATCH_HOTENDS) || isbed == DISABLED(WATCH_HOTENDS) || ischamber == DISABLED(WATCH_HOTENDS)) {
            if (!heated) {                                            // If not yet reached target...
              if (current_temp > next_watch_temp) {                   // Over the watch temp?
               ...
               TmpErrorCnt =0;//2022.3.1 zb
            }
            else if (current_temp < target - (MAX_OVERSHOOT_PID_AUTOTUNE)) // Heated, then temperature fell too far?
            {
              /*2022.3.1 zb*/
              TmpErrorCnt++;
              if(TmpErrorCnt>=TmpErrorNum){
                  TmpErrorCnt =0;
...
void Temperature::manage_heater() {
  ...
  millis_t ms = millis();
    ...
    HOTEND_LOOP() {
	  ...
	  //do{ Serial.print("temp_hotend[e].celsius="); Serial.println(temp_hotend[e].celsius); }while(0);
	  //set_fan_speed(0,255);
	  if(temp_hotend[e].celsius > 45 ) set_fan_speed(1,255);//  1: NO
	  else								set_fan_speed(1,0);
