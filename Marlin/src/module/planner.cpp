...
float Planner::steps_to_mm[DISTINCT_AXES];      // (mm) Millimeters per step
 #ifdef AUTOLEVEL_SWITCH
bool auto_level_enable=true;
#endif
#ifdef FILAMENT_JAMS_SWITCH
bool test_filament_enable=true;
bool test_jams_enable=true;
#endif
...
//extern fast_print_enable_change;

#define ALIM(I,ARR) _MIN(I, (signed)COUNT(ARR) - 1)
static const float	   _DASU[] PROGMEM = DEFAULT_AXIS_STEPS_PER_UNIT;

//extern fast_print_enable;
...
void Planner::change_e_stepper_mm()//float current_position_e
{
  ...
  //do{ Serial.print("position.e:"); Serial.println(position.e); }while(0);
  //do{ Serial.print("ffffcurrent_position.e:"); Serial.println(current_position.e); }while(0);
	...
	if(fast_print_enable_change)
	{
		...
		else
		{
	  	// planner.settings.axis_steps_per_mm[E_AXIS]= pgm_read_float(&_DASU[ALIM(E_AXIS, _DASU)]);
			position.e=current_position.e*settings.axis_steps_per_mm[E_AXIS];
		//	do{ Serial.print("nnnposition.e:"); Serial.println(position.e); }while(0);
		//	do{ Serial.print("nnncurrent_position.e:"); Serial.println(current_position.e); }while(0);
		}
	}
}
