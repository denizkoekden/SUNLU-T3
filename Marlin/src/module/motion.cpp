...
bool Filament_Runout_Check_Flag =false;
/**
...
float max_point[2]={-999.0};
float min_point[2]={999.0};
...
//extern uint8_t FilamentSensorEncoder::Has_Filament_Err;
uint8_t get_has_run_out();


void PrintPausedCheck() {
		while(planner.has_blocks_queued())	planner.synchronize();		
		...
		//do{ Serial.println("PrintPausedCheck1111");	}while(0);
		//do_blocking_move_to_z(current_position.z+5);
		//do_blocking_move_to_z(current_position.z+15);
...
void waitmovecomplete(void) {
	planner.synchronize();
	//do{ Serial.println("set_filament_mm1111");  }while(0);
    	while(planner.has_blocks_queued())	planner.synchronize();	 	
...
//	do{ Serial.println("set_filament_mm1111");  }while(0);
    	while(planner.has_blocks_queued())	planner.synchronize();	 	
	//	do{ Serial.println("set_filament_mm2222");	}while(0);
		while((current_position.x =0)&&(current_position.y =0)&&(current_position.z =20))
			break;

      Filament_Runout_Check_Flag =true;
	  
      const float olde = current_position.e;
      current_position.e += 6;
      line_to_current_position(MMM_TO_MMS(500));
      wait_for_system();
      current_position.e += 6;
      line_to_current_position(MMM_TO_MMS(500));
      wait_for_system();
      ...
      planner.synchronize();  
	 // do{ Serial.print("set_filament_mm_x="); Serial.println(current_position.x); }while(0);
	 // do{ Serial.print("set_filament_mm_y="); Serial.println(current_position.y); }while(0);
	 // do{ Serial.print("set_filament_mm_z="); Serial.println(current_position.z); }while(0);
	  
	//  do{ Serial.print("runout.get_has_error_out(_bbbb="); Serial.println(runout.get_has_error_out()); }while(0);
		if(runout.get_has_error_out()==0){
				ret =true;			  
	}
    else{
...
void do_blocking_move_to(
  ...
  const feedRate_t z_feedrate = fr_mm_s ?: homing_feedrate(Z_AXIS),
    ...
    if (current_position.z > rz) {
      current_position.z = rz;
	  uint16 readz_pin =0;
	  readz_pin = READ(Z_ENABLE_PIN);
		 //  do{ Serial.print("readz_pin222="); Serial.println(readz_pin); }while(0);
