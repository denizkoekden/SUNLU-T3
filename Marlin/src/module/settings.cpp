...
typedef struct SettingsDataStruct {
  ...
  #endif
  bool auto_level_enable;
  bool test_filament_enable;
  bool test_jams_enable;
  bool fast_print_enable;

} SettingsData;

//static_assert(sizeof(SettingsData) <= MARLIN_EEPROM_SIZE, "EEPROM too small to contain SettingsData!");
//bool MarlinSettings::save_par=false,
...
#include "../core/debug_out.h"
  ...
  bool MarlinSettings::eeprom_save_par =false;
  ...
  void MarlinSettings::Period_task() {
	  static millis_t get_cur_time=0,get_last_time=0;  
	 //  do{ Serial.print("lcd_babystep_zoffset_M500_00"); }while(0);
	 
	 //do{ Serial.print("eeprom_save_par="); Serial.println(eeprom_save_par); }while(0);
	 //save_par
	   get_cur_time=millis();
	   if(get_cur_time-get_last_time<5000)
	   {
		   return;	  
	   }
	   else 
  ...
  bool MarlinSettings::save() {
    float dummyf = 0;
    char ver[4] = "ERR";
    do{ Serial.print("save1111"); }while(0);
    ...
    #endif
	#if 1
	EEPROM_WRITE(auto_level_enable);
	EEPROM_WRITE(test_filament_enable);
	EEPROM_WRITE(test_jams_enable);
 	EEPROM_WRITE(fast_print_enable);
  ...
  bool MarlinSettings::_load() {
    ...
    if (strncmp(version, stored_ver, 3) != 0) {
	  ...
	  //do{ Serial.print("recovery_enabled:"); Serial.println(recovery_enabled); }while(0);
	  //do{ Serial.print("recovery.enabled:"); Serial.println(recovery.enabled); }while(0);
      ...
      #if HAS_MULTI_LANGUAGE
      {
		...
		do{ Serial.print("ui_language111="); Serial.println(ui_language); }while(0);
        if (ui_language >= NUM_LANGUAGES) ui_language = 0;
        ui.set_language(ui_language);
		
		do{ Serial.print("ui_language222="); Serial.println(ui_language); }while(0);
      }
      #endif
	  #if 1
  	  EEPROM_READ(auto_level_enable);
	  EEPROM_READ(test_filament_enable);
	  EEPROM_READ(test_jams_enable);
 	  EEPROM_READ(fast_print_enable);
...
void MarlinSettings::reset() {
  ...
  #endif
  ui.set_language(1);
  test_filament_enable=true;//zb
  test_jams_enable    =true;
  auto_level_enable   =true;	
  fast_print_enable_pre=fast_print_enable   =false;	
  //ui.set_language(1);
  //
  // MKS UI controller
  //
