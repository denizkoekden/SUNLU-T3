...
#if 1
  static millis_t tmpIldeTime = millis();
  if (displayNameFlag && IS_SD_PRINTING()) {
    millis_t curIldeTime = millis();
    if (curIldeTime - tmpIldeTime > 10 * 1000) {
      tmpIldeTime = curIldeTime;
      ui.set_status_P(card.filename);
    }
  } 
#endif
